...
import FilterMyList from '../components/FilterMyList';

// Data Asli (Master) - Seolah-olah dari BE
const appliedJobsData = [
  { id: 1, title: "Frontend Dev", company: "Tokopedia", location: "Jakarta", type: "Fulltime", workType: "Remote", salary: "IDR 15jt", tags: ["React", "Tailwind"], status: "Accepted" },
  { id: 2, title: "Backend Engineer", company: "Gojek", location: "Jakarta", type: "Senior", workType: "Hybrid", salary: "IDR 20jt", tags: ["Golang", "SQL"], status: "Rejected" },
  { id: 3, title: "UI/ UX Internship", company: "PT ABCD", location: "Malang", type: "Internship", workType: "On-Site", salary: "Negotiable", tags: ["UI Design", "Figma"], status: "Wait For Response" },
];

const bookmarkedJobsData = [
  { id: 101, title: "UI/ UX Internship", company: "PT ABCD", location: "Malang", type: "Internship", workType: "On-Site", salary: "Negotiable", tags: ["UI Design", "Figma", "Design Systems"] },
  { id: 102, title: "Data Analyst", company: "Shopee", location: "Jakarta", type: "Fulltime", workType: "On-Site", salary: "Negotiable", tags: ["SQL", "Python"] },
  { id: 103, title: "Mobile Dev", company: "Traveloka", location: "Remote", type: "Freshgraduate", workType: "Remote", salary: "Negotiable", tags: ["React Native", "Flutter"] },
  { id: 104, title: "UI/ UX Internship", company: "PT Telkom", location: "Malang", type: "Internship", workType: "Hybrid", salary: "Negotiable", tags: ["UI Design", "Figma"] },
];
...
export default function MyJobList() {
  ...
  const isFiltering = activeFilters.locations.length > 0 || activeFilters.types.length > 0 || searchTerm !== "";

  // Logika filter (useMemo)
  const displayedBookmarks = useMemo(() => {
    return bookmarkedJobs.filter(job => {
      const locationMatch = activeFilters.locations.length === 0 || activeFilters.locations.includes(job.location);
      const typeMatch = activeFilters.types.length === 0 || activeFilters.types.includes(job.type);
    ...
    <div className="min-h-screen bg-gray-50 font-sans pb-10">
      ...
      <main className="max-w-5xl mx-auto px-6 mt-8">
        ...
        <div className="mb-8">
           ...
           <p className="text-slate-500 mt-2 text-lg">Manage your saved jobs, applications, and track your job search progress</p>
         </div>
        ...
        <div className="flex bg-white rounded-xl border border-gray-200 p-1.5 mb-8 shadow-sm">
           <button 
             ...
             className={`flex-1 py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all ${activeTab === 'bookmarked' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}
           >
             Bookmarked
             <span className={`text-xs px-2 py-0.5 rounded-full ${activeTab === 'bookmarked' ? 'bg-white/20 text-white' : 'bg-blue-100 text-blue-600'}`}>
               {displayedBookmarks.length}
             </span>
           </button>
           <button 
             onClick={() => setActiveTab('applied')}
             className={`flex-1 py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all ${activeTab === 'applied' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}
           >
             Applied
             <span className={`text-xs px-2 py-0.5 rounded-full ${activeTab === 'applied' ? 'bg-white/20 text-white' : 'bg-blue-100 text-blue-600'}`}>
